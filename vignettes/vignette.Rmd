...
fragmentation_minimal <-
  rifi_fragmentation(
    probe = penalties_minimal,
    cores = 2
  )
```
<br/><br/>
``` {r echo = TRUE}
head(rowRanges(fragmentation_minimal))
```
<br/><br/>
...
<br/><br/>
``` {r TUgether}
probe_df <- TUgether(probe = penalties_minimal, cores = 2, pen = -0.75)
head(probe_df)
```
<br/><br/>
...
fragmentation_minimal
probe <- predict_ps_itss(data = fragmentation_minimal, maxDis = 300)
...
probe
probe <- apply_t_test(data = probe, threshold = 300)
head(rowRanges(probe))
```
