...
`rifi_wrapper` to run 'rifi" with default options. `rifi_wrapper` conveniently wraps all functions included in rifi. That allows the user to run the whole workflow with one function. If the data contain a background component, e.g. in case of microarray data, 
...
* You may need to adapt the starting values used on the fit to your data

#### 2. Penalties

You may need to check your penalties if the last visualization shows:

* A high frequency of considerable long segments.
<br/><br/>
``` {r longSegments, echo=FALSE, fig.cap="**Long segments**", out.width = '100%'}
knitr::include_graphics("longSegments.png")
```
<br/><br/>

* A high frequency of mini segments.
<br/><br/>
``` {r miniSegments, echo=FALSE, fig.cap="**;ini segments**", out.width = '100%'}
knitr::include_graphics("miniSegments.png")
```
<br/><br/>

* A high frequency of extreme high values.
<br/><br/>
``` {r highValues, echo=FALSE, fig.cap="**High values**", out.width = '100%'}
...
The penalties could be adjusted manually in case you do not detect any anomaly
in row data neither on fit step. 
