...
load("example_input_minimal.RData")
load("example_input_e_coli.RData")
#columns with intensities measurements from all time point and replicates
assay(example_input_e_coli)
#rowRanges of the data
rowRanges(example_input_e_coli)
...
assay(example_input_synechocystis_6803)
#rowRanges of the data
rowRanges(example_input_synechocystis_6803)
# time points
...
#The output
load("rifi_preprocess.rda")
head(rowRanges(preprocess_min))
```
...
``` {r rifi_penalties}
#The output
load("rifi_penalties.rda")
head(rowRanges(penalties_min))
```
<br/><br/>
...
<br/><br/>
``` {r echo = TRUE}
load("rifi_fragmentation.rda")
head(rowRanges(fragmentation_min))
```
<br/><br/>
...
summary_minimal <- rifi_summary(stats_minimal, data_annotation =
                                  annot_g[[1]])
load("summary_minimal.rda")
metadata(summary_minimal)
```
<br/><br/>
...
```
<br/>
The plot is located on vignette "genome_fragments.pdf" and shows shows 4 sections: annotation, delay, half-life and intensity/coverage.
