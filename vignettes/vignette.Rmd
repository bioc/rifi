...
**input_df** is the SummarizedExperiment (SE) input format. SE structure as known 
includes the colData (description of each sample), assay (probes/bins intensity 
...
1.  colData contains 2 columns, timepoint and replicates.
2.  Assay contains the relative intensities need to be numeric. The first column
...
3.  rowRanges includes two columns, "ID" and "position". Each ID in the column
...
print("Rule 4")
input_df_4 <-
  example_input_e_coli[which(example_input_e_coli$strand == "+"), ]
...
result$input_df$filtration[1:6] <-
  c("FLT", "_FLT_", "_FLT_your_text", NA, "_",
    "_passed_")
...
probe based SE **probe_df**. At this stage, The rowRanges contains the columns
...
**ID**, **intensity**, **position** and **position_segment**. `finding_TI`
...
suppressWarnings(fit_minimal <- rifi_fit(
  inp = preprocess_minimal$input_df,
  probe = preprocess_minimal$probe_df,
  cores = 2,
  viz = FALSE,
  details = FALSE,
  restr = 0.2,
...
assay containing **intensity** of all timeserie and rowRanges containing **ID**,
...
<br/><br/>
``` {r plot_nls2, eval=F}
plot_nls2_function(data = fit_minimal[[2]], inp = inp, cores = cores)
```
<br/><br/>
...
plot_singleProbe_function(data = fit_minimal[[3]], inp = inp, cores = cores)
```
<br/><br/>
...
function allows the user in one click to run the whole workflow.
