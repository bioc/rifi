...
<br/><br/>
``` {r make_df}
probe_df <- make_df(inp = result$input_df, cores = 2, bg = 100, rm_FLT = TRUE)
head(probe_df)
```
<br/><br/>
...
preprocess_minimal <- rifi_preprocess(example_input_minimal, cores = 2,
                             ...
                             rm_FLT = FALSE, thrsh_check = 0, dista = 300,
...
fit_minimal <- rifi_fit(inp = preprocess_minimal$input_df,
                     probe = preprocess_minimal$probe_df, cores = 2, viz = FALSE,
                     details = FALSE, restr = 0.2, decay=seq(.08, 0.11, by=.02),
...
`apply_event_position` extract the position between 2 segments with pausing site or iTSS_I event.

### 5. `apply_length_event`
...
The table contains the following columns:
   ...
   **event_duration_[min]**: event duration on minutes. The duration is
    calculated from the intercept of two delay fragments at the position 0.
...
The arguments used are:
   ...
   **event_duration_ps** integer: threshold for pausing sites selected to
   plot, default is -2.
   
   **event_duration_itss** integer: threshold for itss_I selected to plot,
   default is 2.
...
``` {r rifi_visualization}
 ...
 HL_threshold = 20, vel_threshold = 200, ps_color = "orange", iTSS_I_color = "blue")
```
<br/><br/>
