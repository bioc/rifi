#' apply_event_duration: is a short version of apply_Ttest_delay function
#' to extract event time duration as pausing site or iTSS happens.
#'
#' apply_event_duration adds a new column with the duration.
#'
...
#' @examples
#' data(stats_minimal)
#' apply_event_duration(data = stats_minimal)
#' 
#' @export
#'

apply_event_duration <- function(data) {
  ...
  for (i in seq_along(event)) {
    ...
    if (length(seg_1_d) == 1 | length(seg_2_d) == 1) {
      ...
      data[which(data$ps_ts_fragment %in% ps),
           "event_duration"] <- abs(del_p1 - del_p2)
      data[which(data$ps_ts_fragment %in% ps),
           "event_position"] <-
